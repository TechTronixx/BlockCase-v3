    ...
    try {
      ...
      if (empty(divDashboardBudgetList.getAttribute("data-month"))) {
        ...
        currentMonth = parseFloat(
          divDashboardBudgetList.getAttribute("data-month")
        );
      }
      ...
      if (empty(divDashboardBudgetList.getAttribute("data-year"))) {
        ...
        currentYear = parseFloat(
          divDashboardBudgetList.getAttribute("data-year")
        );
      }
...
async function populateDashboardWatchlist(recreate) {
  if (getActivePage().id === "dashboard-page") {
    ...
    try {
      ...
      let marketCryptoData = !empty(watchlistCryptoIDs)
        ? await cryptoAPI.getMarketByID(currency, watchlistCryptoIDs.join(","))
        : {};
      ...
      if (
        divDashboardWatchlistList.getElementsByClassName("loading-icon")
          .length > 0 ||
        divDashboardWatchlistList.childElementCount !== rows.length
      ) {
      ...
      for (let i = 0; i < rows.length; i++) {
        if (divDashboardWatchlistList.childElementCount >= i + 1) {
          let current =
            divDashboardWatchlistList.getElementsByClassName(
              "watchlist-list-row"
            )[i];
  ...
  Object.keys(categories).map((category) => {
    ...
    labels.push(
      `  ${capitalizeFirstLetter(category)}: ${
        currencySymbols[currency] + separateThousands(amount)
      }`
    );
...
async function generateBudgetStats(budgetData, transactionData, recreate) {
  ...
  if (earned > 0) {
    spanIncome.textContent = `Based on your transactions, aside from your income of ${
      currencySymbols[currency] +
      separateThousands(parseFloat((income / 12).toFixed(0)))
    }, you earned an additional ${
      currencySymbols[currency] + separateThousands(earned)
    } this month.`;
  } else {
    spanIncome.textContent = `Based on your transactions, you didn't earn any additional money aside from your income of ${
      currencySymbols[currency] +
      separateThousands(parseFloat((income / 12).toFixed(0)))
    }.`;
  }
  ...
  Object.keys(categories).map((category) => {
    ...
    let remainingPercentage = parseFloat(
      ((remaining * 100) / amount).toFixed(0)
    );
    ...
    if (span.id in tippyInstances.budgetStats) {
      tippyInstances.budgetStats[span.id].setContent(
        `Used: ${currencySymbols[currency] + separateThousands(used)}`
      );
      tippyInstances.budgetStats[div.id].setContent(
        `Used: ${currencySymbols[currency] + separateThousands(used)}`
      );
      tippyInstances.budgetStats[div.id + "-background"].setContent(
        `Remaining: ${currencySymbols[currency] + separateThousands(remaining)}`
      );
    } else {
      tippyInstances.budgetStats[span.id] = tippy(span, {
        content: `Used: ${currencySymbols[currency] + separateThousands(used)}`,
        placement: "bottom",
      });
      tippyInstances.budgetStats[div.id] = tippy(div, {
        content: `Used: ${currencySymbols[currency] + separateThousands(used)}`,
        placement: "bottom",
      });
      tippyInstances.budgetStats[div.id + "-background"] = tippy(
        div.parentElement.getElementsByClassName("background")[0],
        {
          content: `Remaining: ${
            currencySymbols[currency] + separateThousands(remaining)
          }`,
          placement: "right",
        }
      );
...
function addBudgetDateEvents(buttonMonth, buttonYear) {
  buttonMonth.addEventListener("click", () => {
    ...
    let popup = new Popup(360, "auto", "Select Month", html, {
      confirmText: "-",
      cancelText: "Dismiss",
      page: "dashboard",
    });
    ...
    for (let i = 0; i < choices.length; i++) {
      choices[i].addEventListener("click", () => {
        ...
        divDashboardBudgetList.setAttribute(
          "data-month",
          monthNames.indexOf(choices[i].textContent)
        );
  ...
  buttonYear.addEventListener("click", () => {
    ...
    let popup = new Popup(360, "auto", "Select Year", html, {
      confirmText: "-",
      cancelText: "Dismiss",
      page: "dashboard",
    });
...
function filterTransactionsByMonth(transactionData, month, year) {
  ...
  Object.keys(transactionData).map((key) => {
    try {
      ...
      if (
        parseFloat(month) === date.getMonth() &&
        parseFloat(year) === date.getFullYear()
      ) {
...
async function listTransactions() {
  try {
    ...
    addTransactionSearchEvent(
      document.getElementById("input-search-transaction"),
      document.getElementById("button-search-transaction")
    );
    addTransactionButtonEvent(
      document.getElementById("button-add-transaction")
    );
    ...
    keys.map((key) => {
      ...
      div.setAttribute(
        "class",
        `transaction-row noselect ${transaction.transactionCategory}`
      );

      let date =
        choices?.dateFormat === "dd-mm-yyyy"
          ? formatDateHyphenatedHuman(
              new Date(Date.parse(transaction.transactionDate))
            )
          : formatDateHyphenated(
              new Date(Date.parse(transaction.transactionDate))
            );
      ...
      div.innerHTML = `
				...
				<div class="item">
					<span class="category">${capitalizeFirstLetter(
            transaction.transactionCategory
          )}</span>
				</div>
				<div class="item">
					<span class="type ${transaction.transactionType}">${
        transaction.transactionCategory === "savings"
          ? "Saved"
          : capitalizeFirstLetter(transaction.transactionType)
      } ${
        currencySymbols[currency] +
        separateThousands(transaction.transactionAmount)
      }</span>
				</div>
			`;

      if (
        !empty(transaction.transactionNotes) &&
  ...
  try {
    div.addEventListener("click", () => {
      let html = `
				...
				<input id="popup-input-amount" type="number" placeholder="Amount..." spellcheck="false" autocomplete="off" value="${
          transaction.transactionAmount
        }">
				<div class="popup-button-wrapper margin-bottom">
					<button id="popup-choice-earned" class="choice ${
            transaction.transactionType === "earned" ? "active" : ""
          }">Earned</button>
					<button id="popup-choice-spent" class="choice ${
            transaction.transactionType === "spent" ? "active" : ""
          }">Spent</button>
				</div>
				<span class="popup-input-span">Category</span>
				<input class="audible-pop" id="popup-input-category" type="text" placeholder="Category..." autocomplete="off" spellcheck="false" readonly value="${capitalizeFirstLetter(
          transaction.transactionCategory
        )}">
				<span class="popup-input-span">Date</span>
				<input id="popup-input-date" type="text" placeholder="Date..." autocomplete="off" spellcheck="false" value="${
          transaction.transactionDate
        }">
				<span class="popup-input-span">Notes</span>
				<input id="popup-input-notes" type="text" placeholder="Notes..." autocomplete="off" value="${
          transaction.transactionNotes
        }">
				<button class="action-button delete" id="popup-button-delete-transaction">Delete Transaction</button>
			`;

      let popup = new Popup(300, "auto", "Update Transaction", html, {
        confirmText: "Add",
        page: "dashboard",
      });
      popup.show();
      ...
      addTransactionPopupDeleteEvent(
        popup,
        document.getElementById("popup-button-delete-transaction"),
      ...
      popup.on("confirm", async () => {
        ...
        let data = parseTransactionPopupData(
          popupInputAmount,
          popupChoiceEarned,
          popupInputCategory,
          popupInputDate,
        ...
        await updateTransaction(
          ...
          userID,
          transaction.transactionID,
          encrypted.transactionType,
          encrypted.transactionDate,
          encrypted.transactionCategory,
          encrypted.transactionAmount,
          encrypted.transactionNotes
        );
...
function addTransactionPopupDeleteEvent(
  previousPopup,
  buttonDelete,
  transactionID
) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (values.join(",").includes(query)) {
      if (
        empty(firstFound) &&
        rows[i] !== rows[0] &&
        divSideMenuContainer.getElementsByClassName("first-found").length ===
          0 &&
...
function addTransactionButtonEvent(button) {
  button.addEventListener("click", () => {
    try {
      ...
      let popup = new Popup(300, "auto", "Add Transaction", html, {
        confirmText: "Add",
        page: "dashboard",
      });
      popup.show();
...
function validateTransactionData(amount, type, category, date, notes) {
  try {
    ...
    if (
      empty(category) ||
      (!Object.keys(defaultBudgetData.categories).includes(
    ...
    if (
      type === "earned" &&
      !["income", "savings"].includes(category.toLowerCase())
    ) {
      return {
        error: "Category must be set to Income or Savings if you earned money.",
      };
    }
    ...
    return {
      ...
      transactionCategory: category.toLowerCase(),
...
function parseTransactionPopupData(
  popupInputAmount,
  popupChoiceEarned,
  popupInputCategory,
  popupInputDate,
  ...
  try {
    ...
    let type = popupChoiceEarned.classList.contains("active")
      ? "earned"
      : "spent";
...
function addTransactionCategoryEvent(previousPopup, input) {
  ...
  input.addEventListener("click", () => {
    ...
    if (
      document.getElementById("popup-choice-spent").classList.contains("active")
    ) {
      html = `
				<div class="popup-button-wrapper no-margin-top">
    ...
    let popup = new Popup(360, "auto", "Transaction Category", html, {
      confirmText: "-",
      cancelText: "Back",
      page: "dashboard",
    });
...
async function createWatchlistListRows(
  marketCryptoData,
  marketStocksData,
  watchlistData
) {
      ...
      for (let i = 0; i < ids.length; i++) {
        try {
          ...
          if (asset.assetType === "crypto") {
            ...
            div.setAttribute(
              "class",
              "watchlist-list-row crypto noselect audible-pop"
            );
            ...
            div.innerHTML = `
                      ...
                      currencySymbols[currency] + abbreviateNumber(marketCap, 2)
                    }</span>
									</div>
            ...
            div.setAttribute(
              "class",
              "watchlist-list-row stock noselect audible-pop"
            );
...
function addWatchlistDeleteEvent(div, asset) {
  div
    .getElementsByClassName("action-button delete")[0]
    .addEventListener("click", async () => {
      ...
      let popup = new Popup(
        ...
        "auto",
        "Delete Asset",
        `<span>Are you sure you want to remove ${asset.assetSymbol.toUpperCase()} from your watchlist?</span>`,
        { page: "dashboard" }
      );
      popup.show();
      popup.updateHeight();
  ...
  div.addEventListener("click", async (event) => {
    if (!event.target.classList.contains("delete")) {
      ...
      if (asset.assetType === "crypto") {
        ...
        let resultPrice = await fetchStockPrice(
          currency,
          [asset.assetSymbol],
          true
        );
        ...
        let resultHistorical = await fetchStockHistorical(
          currency,
          asset.assetSymbol,
          true
        );
        ...
        let infoHistorical =
          resultHistorical.data.historicalData.chart.result[0];
...
function getWatchlistIDBySymbol(watchlist, symbol, type) {
  try {
    ...
    Object.keys(watchlist).map((index) => {
      let asset = watchlist[index];
      if (
        asset?.assetSymbol.toLowerCase() === symbol.toLowerCase() &&
...
function setDefaultBudgetData() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let encrypted = CryptoFN.encryptAES(
        JSON.stringify(defaultBudgetData),
        key
      );
